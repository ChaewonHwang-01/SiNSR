        ...
        if self.rank == 0:
            ...
            for ii, data in enumerate(self.dataloaders[phase]):
                ...
                with torch.no_grad():
                    ...
                    # [-1, 1] → [0, 1] 로 바꾸고, 혹시 넘치는 값은 잘라주기
                    img_01 = results * 0.5 + 0.5
                    img_01 = torch.clamp(img_01, 0.0, 1.0)

                    # pyiqa metric에는 [0,1] 범위 이미지만 넣기
                    mean_clipiqa += self.metric_dict["clipiqa"](img_01).sum().item()
                    mean_musiq   += self.metric_dict["musiq"](img_01).sum().item()
    ...
    def validation(self, phase='val'):
        ...
        if self.rank == 0:
            ...
            for ii, data in enumerate(self.dataloaders[phase]):
                ...
                # --- 공통: [-1,1] -> [0,1]로 바꾸고 범위 클램핑 (추가함)---
                results_01 = torch.clamp(results.detach() * 0.5 + 0.5, 0.0, 1.0)
                """
                if 'gt' in data:
                    mean_psnr += util_image.batch_PSNR(
                ...
                with torch.no_grad():
                    ...
                    mean_musiq += self.metric_dict["musiq"](results.detach() * 0.5 + 0.5).sum().item()"""
                if 'gt' in data:
                    gt_01 = torch.clamp(im_gt * 0.5 + 0.5, 0.0, 1.0)
                    ...
                    # LPIPS: 원래 코드처럼 [-1,1] 그대로 사용 (클램핑 X)
                    mean_lpips += self.lpips_loss(results.detach(), im_gt).sum().item()

                with torch.no_grad():
                    # CLIPIQA / MUSIQ는 반드시 [0,1]이어야 해서 clamp 된 값 사용
                    mean_clipiqa += self.metric_dict["clipiqa"](results_01).sum().item()
                    mean_musiq  += self.metric_dict["musiq"](results_01).sum().item()
                if (ii + 1) % self.configs.train.log_freq[2] == 0:
                    self.logger.info(f'Validation: {ii+1:02d}/{num_iters_epoch:02d}...')
